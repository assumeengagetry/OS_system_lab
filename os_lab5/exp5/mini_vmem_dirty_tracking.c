...
int main(void) {
    ...
    printf("mini_vmem Dirty Tracking start: %d virtual pages, only %d resident pages allowed\n",
           NUM_VPAGES, MAX_RESIDENT);

    printf("\nread page 0 only, it should stay clean\n");
    printf("page 0 value = %d\n", vm_base[0]);

    printf("\nread page 1 only, it should stay clean\n");
    printf("page 1 value = %d\n", vm_base[page_size]);

    printf("\nwrite page 2 = C, first write should trigger [dirty]\n");
    vm_base[2 * page_size] = 'C';

    printf("\nwrite page 3 = D, first write should trigger [dirty]\n");
    vm_base[3 * page_size] = 'D';

    printf("\nwrite page 4 = E, eviction should choose clean page 0 and avoid swap write\n");
    vm_base[4 * page_size] = 'E';

    printf("\nwrite page 5 = F, eviction should choose clean page 1 and avoid swap write\n");
    vm_base[5 * page_size] = 'F';

    printf("\nforce more evictions, dirty pages should be written to swap\n");
    vm_base[6 * page_size] = 'G';
    vm_base[7 * page_size] = 'H';

    printf("\nrevisit page 2, should swap-in and still be C\n");
    printf("page 2 value = %c\n", vm_base[2 * page_size]);
...
static void segv_handler(int sig, siginfo_t *info, void *context) {
    ...
    if (page_table[page_no].state == PAGE_RESIDENT && page_table[page_no].dirty == 0) {
        if (mprotect(page_start(page_no), page_size, PROT_READ | PROT_WRITE) == -1) {
            perror("mprotect dirty");
            exit(1);
        }
        page_table[page_no].dirty = 1;
        printf("[dirty] page %d first write, permission -> RW\n", page_no);
        return;
    }
...
static void load_page(int page_no) {
    ...
    if (mprotect(addr, page_size, PROT_READ | PROT_WRITE) == -1) {
        perror("mprotect load temp RW");
        exit(1);
    }

    if (page_table[page_no].state == PAGE_SWAPPED && page_table[page_no].in_swap) {
...
static void evict_page(int victim) {
    ...
    if (page_table[victim].dirty) {
        ...
        page_table[victim].in_swap = 1;
        printf("[evict] dirty page %d -> swap offset %ld\n", victim, (long)offset);
    } else {
        printf("[evict] clean page %d, no swap write\n", victim);
    }
    ...
    page_table[victim].state = page_table[victim].in_swap ? PAGE_SWAPPED : PAGE_EMPTY;
