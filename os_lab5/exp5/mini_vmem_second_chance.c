...
static char vmem_read(int page_no, size_t offset);
static void vmem_write(int page_no, size_t offset, char value);

int main(void) {
    init_vmem();

    printf("mini_vmem Second Chance start: %d virtual pages, only %d resident pages allowed\n",
    ...
    printf("\nwrite page 4 = E, all pages initially referenced, now we should use second chance and the first is A\n");
    vmem_write(4, 0, 'E');

    printf("\nread page 1 again, mark it referenced before next eviction\n");
    printf("page 1 value = %c\n", vmem_read(1, 0));

    printf("\nwrite page 5 = F, Second Chance should skip referenced page 1\n");
    vmem_write(5, 0, 'F');

    printf("\nrevisit page 0, and it  will swap-in 4 it  evicted\n");
...
static char vmem_read(int page_no, size_t offset) {
    char value = vm_base[(size_t)page_no * page_size + offset];
    page_table[page_no].referenced = 1;
printf("[now]now the reference is 1\n");
    return value;
}

static void vmem_write(int page_no, size_t offset, char value) {
    vm_base[(size_t)page_no * page_size + offset] = value;
    page_table[page_no].referenced = 1;
...
static void load_page(int page_no) {
    ...
    if (resident_count >= MAX_RESIDENT) {
        int victim = choose_victim_second_chance();
        if (victim < 0) {
    ...
    if (page_table[page_no].state == PAGE_SWAPPED) {
        ...
        printf("[load] page %d <- swap offset换出 %ld\n", page_no, (long)offset);
    } else {
    ...
    page_table[page_no].dirty = 1;
    page_table[page_no].referenced = 0;
...
static void evict_page(int victim) {
    ...
    printf("[evict] page %d -> swap offset换出 %ld\n", victim, (long)offset);
}
